...
class PowerLoadModel:
    def __init__(self, data_path):
        ...
        # print('时间对应的load的字典',self.time_key_load)
    ...
    def ana_data(self):
        ...
        # 提取小时信息，pandas中series获取小时信息，series.dt.属性；pandas获取单个str类型时间数据，pd.to_datetime(time_str).属性
    ...
    def feature_engineering(self):
        ...
        # 获取不到返回None，最前面1天和前3个小时的数据会产生None
        # 使用time - load 字典获取前一天对应时刻的负荷
        # last_1_load = last_1_day.apply(lambda x: self.time_key_load.get(x))
        last_1_load = last_1_day.map(self.time_key_load)
        ...
        self.logger.info('')
        # 返回特征，标签，别名
        return data.iloc[:, 2:], data.iloc[:, 2], data.columns[2:]
    ...
    def model_train(self):
        ...
        x_train, x_val, x_test = (x.iloc[:train_size, :], x.iloc[train_size:test_size+val_size,:],
                                x.iloc[train_size + val_size:, :])

        y_train, y_val, y_test = (y.iloc[:train_size], y.iloc[train_size:test_size + val_size],
                                y.iloc[train_size + val_size:])

        # # 2:网格化搜索与交叉验证
        # # 定义xgb模型，指定超参数：迭代数数量的上限1000，早停(防止过拟合)轮数： 连续20轮训练指标没有提升，就停止训练
        ...
        # }
        # # 定义事件序列交叉验证： 需要使用TimeSeriesSplit方法，防止信息泄露
        ...
        # 3.模型实例化,模型训练(xgb网格搜索后，需要把训练集和验证集一起用最有超参数再重新训练一下)
