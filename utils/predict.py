...
class PowerLoadPredict(object):
    ...
    def pred_feature_extract(self,time_key,time_key_load = None):
        ...
        #2.解析时间窗口特征
        # 这里有两种情况，一种是offline(回归)测试，需要对未来time_key_load进行掩码，需要单独传进来
        ...
        # 4: 构造pandas和DataFrame
        feature_columns = ['前1小时', '前2小时', '前3小时', 'yesterday_load', 'hour_0', 'hour_1', 'hour_2', 'hour_3',
        ...
        feature_list = [last_1_hour_load,last_2_hour_load,last_3_hour_load,last_1_day_load,*hour_list,*month_list]
    ...
    def model_predict(self,time_key = None):
        if time_key is not None:
            ...
            self.logger.info(f'==========(online)开始离线预测{time_key}负荷==========')
            #4.1 确定要预测的时间段（2015-08-01 00:00:00及以后的时间）
            data = self.source_data.copy()
            data = data[data.time > '2015-8-1 00:00:00']
            ...
            self.logger.info(f'==========(offline)离线预测评估MAE指标{mae}==========')
            self.logger.info('==========(offline)结束离线预测=========')
